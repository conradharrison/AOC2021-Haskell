...
nextNodes (Graph ns es) (Node s _ _) = L.filter (\(Node name count _) -> ((isConnected es s name) && (name/="start") && ((count==0 && (nameIsLower name))||(nameIsUpper name)||(name=="end")))) ns
...
walk g (Node "end" _ _) = (g, [["end"]])
walk (Graph ns es) (Node s c l) = foldl (\((Graph nss ess), x) (Node ss cc ll) -> 
