...
main = do
        ...
        --let (g, n) = fireBoard (incBoard (fromList (map makeMap (lines inStr)))) 0 in (putStrLn $ (show n ++ "-\n-" ++ show g))
        --let (g, n) = fireBoard (incBoard (fromList (map makeMap (lines inStr)))) 0 in (let (g',n')=(fireBoard (incBoard g) 0) in (putStrLn $ (show n' ++ "-\n-" ++ formatGrid g')))
