...
liesOnLine p s = case (x1==x2, y1==y2) of 
                    (True , True ) -> (x==x1) && (y==y1)
                    (True , False) -> (x==x1) && (((y-y1)*(y-y2))<=0)
                    (False, True ) -> (y==y1) && (((x-x1)*(x-x2))<=0)
                    (False, False) -> (((y-y1)==(x-x1)) || ((y-y1)==(x1-x))) && (((y-y1)*(y-y2))<=0) && (((x-x1)*(x-x2))<=0)
                 where ((Point x y), (Segment (Point x1 y1) (Point x2 y2))) = (p, s)
...
main = do
        ...
        print $ result $ readSegments $ lines inStr
